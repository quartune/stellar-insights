# API Strategy: REST Primary, GraphQL Deprecated

## Overview
REST API is now the single source of truth (/api/v1). GraphQL deprecated (feature-flagged, disabled by default).

**REST Use Cases** (see backend/services/api_guidelines.md):
- Public/simple queries/caching
- Complex analytics
- Real-time via WS

**GraphQL Status**: Disabled. Enable with `--features graphql-deprecated`. Remove in 3 months.

## Implementation
- GraphQL files guarded with `#[cfg(feature = \"graphql-deprecated\")]`.
- No router integration found.
