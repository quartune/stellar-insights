...
pub fn compute_corridor_analytics(payments: &[PaymentRecord]) -> Vec<CorridorAnalytics> {
    ...
    for payment in payments {
        ...
        corridor_payments.entry(corridor_key).or_default().push(payment);
    }
    ...
    for (corridor_key, corridor_payment_records) in corridor_payments {
        ...
        let successful_transactions = corridor_payment_records.iter()
        ...
        let volume_usd: f64 = corridor_payment_records.iter()
            .map(|p| p.amount)
            .sum();
...
pub fn compute_corridor_analytics_for_date(
    ...
    let filtered_payments: Vec<PaymentRecord> = payments
        .iter()
        .filter(|p| {
            p.timestamp.date_naive() == target_date.date_naive()
        })
        .cloned()
        .collect();
...
mod tests {
    ...
    fn test_compute_corridor_analytics_basic() {
        ...
        assert_eq!(corridor_analytics.success_rate, 66.66666666666667);
