...
use backend::analytics::corridor::{
    compute_corridor_analytics, compute_corridor_analytics_for_date,
    get_top_corridors_by_volume, get_top_corridors_by_transactions, get_corridors_by_success_rate
};
...
fn test_known_dataset_expected_success_rates() {
    ...
    let usdc_eurc_analytics = analytics.iter()
    ...
    let usdc_btc_analytics = analytics.iter()
...
fn test_all_failed_payments() {
    let base_time = Utc::now();
    let payments = vec![
        create_test_payment("USDC", "anchor1", "EURC", "anchor2", 100.0, false, base_time),
...
fn test_date_filtering() {
    ...
    let payments = vec![
        create_test_payment("USDC", "anchor1", "EURC", "anchor2", 100.0, true, base_time),
        create_test_payment("USDC", "anchor1", "EURC", "anchor2", 50.0, true, yesterday_time),
        create_test_payment("EURC", "anchor2", "USDC", "anchor1", 75.0, false, yesterday_time),
    ];
...
fn test_corridor_normalization_consistency() {
    ...
    let payments_forward = vec![
        create_test_payment("USDC", "anchor1", "EURC", "anchor2", 100.0, true, base_time),
    ];
    
    let payments_reverse = vec![
        create_test_payment("EURC", "anchor2", "USDC", "anchor1", 100.0, true, base_time),
    ];
    ...
    assert_eq!(analytics_forward[0].total_transactions, analytics_reverse[0].total_transactions);
    assert_eq!(analytics_forward[0].success_rate, analytics_reverse[0].success_rate);
}
